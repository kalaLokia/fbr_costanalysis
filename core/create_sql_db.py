...
def createBomDB(bom_df, items_df) -> tuple:
    unwanted_columns = [
        # "Father Name",
        "Father No of pairs",
        "Father Qty",
    ...
    bom_df["father"] = bom_df.father.str.upper()

    art_df = create_article_list(bom_df)
    # bom_df.to_csv("data/db.csv", index=False)
    engine = create_engine("sqlite:///bom.db", echo=False)

    bom_df.to_sql("bom", con=engine, if_exists="replace", index=False)
    art_df.to_sql("artlist", con=engine, if_exists="replace", index=False)
